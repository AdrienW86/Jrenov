      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {realisations.map((item) => (
            <article key={item.slug} className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden flex flex-col justify-between hover:shadow-md transition">
              <div className="space-y-4 p-6">
                ...
                <h2 className="text-xl font-bold text-slate-900 leading-snug hover:text-amber-600 transition">
                  <Link href={`/realisations/${item.slug}`}>{item.title}</Link>
                </h2>
                <p className="text-slate-600 text-xs sm:text-sm leading-relaxed">{item.summary}</p>
                <div className="flex items-center gap-4 text-xs text-slate-500 pt-2 border-t border-slate-100">
                  <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-amber-500" />{item.city}</span>
                  <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-amber-500" />{item.duration}</span>
                </div>
              </div>
